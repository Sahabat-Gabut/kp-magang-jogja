...
class ProjectController extends Controller {
    ...
    public function detail($id) {
        $team       = TeamApprentice::where("status_hired","DI TERIMA")->where("id",$id)->get();
        $progress   = ProgressProject::where("project_id",$team[0]->project->id)->with('jss')->get();
        $countProgress = ProgressProject::with(["project"])->where("project_id", $team[0]->project->id)->count();
        $done       = ProgressProject::with(["project"])->where("project_id", $team[0]->project->id)->where("status","SELESAI")->count();

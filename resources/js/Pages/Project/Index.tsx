...
export default function Project() {
    ...
    const _onClick = (apprentice_id: number, name: string, id: number, status: string, fullname: string) => {
        ...
        setEdit({ id: id, name: name, apprentice_id: apprentice_id, status: status, project_id: project.id, fullname: fullname });
    };
    return (
        <>
            {projects && (
                                            ...
                                            {projects.data.map(({ name, team, id }, key) => (
                                                <tr key={key} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                                        <InertiaLink href={route('project.show', id)} className="text-gray-600 hover:text-gray-900">
                                                            Detail
                                                        </InertiaLink>
                                                    </td>
                                                </tr>
                ...
                <>
                    ...
                    <div className="w-full mt-10 overflow-x-auto border border-gray-300 rounded-lg h-ful">
                        <table className="w-full h-full rounded-lg whitespace-nowrap">
                            ...
                            <tbody className="flex flex-col items-center w-full overflow-y-auto text-sm font-light text-gray-800 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-50" style={{ maxHeight: '39vh' }}>
                                        ...
                                        <tr className="flex w-full border-b last:border-b-0 hover:bg-gray-50" key={idx}>
                                            ...
                                            <td className="w-1/4 px-5 py-2">
                                                <div className="flex justify-center text-lg rounded-lg" role="group">
                                                    <InertiaLink href={route('progress.show', id)} as="button" className="px-4 py-2 mx-0 text-gray-600 bg-white border border-r-0 border-gray-300 outline-none hover:bg-green-200 hover:text-green-600 rounded-l-md">
                                                    ...
                                                    <button onClick={() => _onClick(apprentice_id, name, id, status, jss.fullname)} className={`px-4 py-2 mx-0 text-gray-600 bg-white border border-l-0 border-gray-300 outline-none rounded-r-md ${apprentice_id !== auth.user?.apprentice.id ? 'cursor-not-allowed hover:bg-gray-200' : 'hover:bg-yellow-200 hover:text-yellow-600'}`} disabled={apprentice_id === auth.user?.apprentice.id ? false : true}>
                                                        <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                    </button>
                                                    <Confirm
                                                        title="Ubah Planning"
                                                        open={editOpen}
...
Project.layout = (page: React.ReactChild) => <AppLayout children={page} />;
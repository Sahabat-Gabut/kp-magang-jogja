...
const FormikInputFile = ({label, helperText, ...props}: Props) => {
    ...
    return (
        <div className="w-full">
            ...
            <div
                className={`relative h-10 border border-gray-400 border-dashed rounded-lg mt-1 text-sm w-full bg-gray-50 focus:outline-none focus:border-green-600 ${meta.touched && meta.error ? 'border-red-300' : ''}`}>
                <label htmlFor={props.name}
                       className="absolute w-full h-full px-3 py-2 overflow-hidden font-mono text-right cursor-pointer">
                    {file ?
                        <span className="h-4 overflow-hidden overflow-ellipsis whitespace-nowrap">{fileName}</span> :
                        <span className="hover:underline">unggah berkas</span>}
                </label>
            </div>

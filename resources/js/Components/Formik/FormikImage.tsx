...
const FormikImage = ({label, readOnly, ...props}: Props) => {
        ...
        <div className="mr-5 h-36 w-36">
            <div className="relative flex">
                <label htmlFor={props.name} className="relative cursor-pointer group">
                    <div
                        className={`relative flex mx-auto pb-5 bg-gray-100 border rounded-lg h-36 w-36 ${isError ? 'border-red-300' : 'border-gray-300'}`}>
                        <span className={`flex w-full bg-gray-100 rounded-lg h-32 ${file && src && 'hidden'}`}>
                            <svg className="mx-auto w-36 h-36" fill="#b1b2b6" viewBox="0 0 20 20"
                                 xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd"
                                                                          d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"
                                                                          clipRule="evenodd"></path></svg>
                        </span>
                        {file && src &&
                        <img src={src.result} alt={file.name} className="object-cover w-full rounded-lg h-36"/>}
                        <div
                            className="absolute mx-auto rounded-lg h-36 w-36 md:mx-0 group-hover:bg-black group-hover:bg-opacity-20"></div>
                        <div className="absolute flex w-full h-full opacity-0 group-hover:opacity-100">
                            <svg className="w-6 h-6 m-auto" fill="none" stroke="#fff" viewBox="0 0 24 24"
